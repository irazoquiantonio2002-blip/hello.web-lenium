document.addEventListener('DOMContentLoaded', () => {
        ...
        class Particle {
            ...
            draw() {
                ...
                ctx.fillStyle = this.red
                    ? `rgba(14,165,233,${this.a * 0.95})`
                    : `rgba(0,0,0,${this.a})`;
                ctx.fill();
                if (this.glow) {
                    ctx.beginPath();
                    ctx.arc(this.x, this.y, this.r * 3.2, 0, Math.PI * 2);
                    ctx.fillStyle = `rgba(14,165,233,${this.a * 0.16})`;
                    ctx.fill();
                }
            }
        }
    ...
    if (form) {
        form.addEventListener('submit', e => {
            ...
            let txt = `Hola Milenium by MAPRO, soy *${name}*.\n`;
            ...
            setTimeout(() => {
                window.open(`https://wa.me/5219984050326?text=${encodeURIComponent(txt)}`, '_blank');
                form.reset();
...
function initTypewriter() {
    ...
    if (!el) return;
    // All words make grammatical sense: "que [word] tu Marca"
    const words = ['Transforman', 'Impulsan', 'Distinguen', 'Multiplican', 'Consolidan', 'Posicionan'];
